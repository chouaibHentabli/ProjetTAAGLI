...
public class ModificationResource {
    ...
    public ResponseEntity<ModificationDTO> createModification(@RequestBody ModificationDTO modificationDTO) throws URISyntaxException {
        ...
        if (modificationDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("modification", "idexists", "A new modification cannot already have an ID")).body(null);
        }
        ...
        return ResponseEntity.created(new URI("/api/modifications/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("modification", result.getId().toString()))
            .body(result);
    }
     ...
     * or with status 500 (Internal Server Error) if the modificationDTO couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @RequestMapping(value = "/modifications",
    ...
    public ResponseEntity<ModificationDTO> updateModification(@RequestBody ModificationDTO modificationDTO) throws URISyntaxException {
        ...
        if (modificationDTO.getId() == null) {
            return createModification(modificationDTO);
        }
        ModificationDTO result = modificationService.save(modificationDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert("modification", modificationDTO.getId().toString()))
            .body(result);
    }
     ...
     */
    @RequestMapping(value = "/modifications/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<ModificationDTO> getModification(@PathVariable Long id) {
        log.debug("REST request to get Modification : {}", id);
        ModificationDTO modificationDTO = modificationService.findOne(id);
        return Optional.ofNullable(modificationDTO)
    ...
    public ResponseEntity<Void> deleteModification(@PathVariable Long id) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("modification", id.toString())).build();
    }
